...
OneMap runs across every default-eligible HDB, executive-condominium, and private non-landed postal point directly. Private landed homes are retained as residential index records but excluded from scheduled mapping because the current study does not claim a defensible route origin for every landed address. No condo or EC postcode inherits another postcode's route result: each eligible postal coordinate has its own three-observation job set. This higher-resolution policy is intentional because a result may inform a housing or hostel decision, where a development-level representative can hide entrance and walking-access differences.
...
The completed local source build contains 94,334 unique postal-code origins: 10,796 HDB records and 83,538 URA records. Three URA postal points overlap an HDB postal point and are retained under the HDB source, leaving 83,538 separate URA rows. The strict audit found zero duplicate postal keys and zero invalid coordinates. The direct-postal routing population contains 10,796 HDB, 603 executive-condominium, and 9,896 private non-landed postal points: 21,295 routed origins and 63,885 OneMap observations at three departure times. The 73,039 private-landed points remain searchable but are excluded from scheduled OneMap routing.
